...
| **[6]** | **NTB:** Populate personal/employment/income details card & allow modifications | Displayed via [PersonalDetailsWidget.tsx](file:///d:/workflow-agent/AiAgentChat/frontend/src/components/widgets/PersonalDetailsWidget.tsx) in `identity/personal_details`. | ⚠️ | **Missing Modification Flow:** Excel specifies a sub-flow (Modifying Personal, Address, Employment, or Income details) with document uploads and Open Banking linking that is currently ignored. |
| **[7]** | **NTB:** Ask customer for average monthly expenses across categories | Completely skipped. Journey transitions straight to offer step. | ❌ | **Missing Step:** We must build a card/widget to collect customer monthly expenses (e.g. housing, utilities, transportation, debt). |
