...
| `persistence.py` | JSON file-based session/conversation store. Sessions expire after 30 min TTL. |
| `graph/graph.py` | Defines the LangGraph `StateGraph`: `classify → extract → respond → END` |
| `graph/state.py` | `ConversationState` TypedDict: messages, session, last_response, extract, intent, classified_data |
...
| `extractors/router.py` | Session state machine + Temporal signal routing. Contains `_advance_session_state()` which implements the full 5-step state machine. |
...
- **build_response:** Full LLM call with dynamic system prompt → customer-facing message

#### Session State Machine (Sub-steps)

```
identity: awaiting_id → nafath_pending → loading → verified → personal_details
offer:    eligible → slider → summary
trade:    loading → success
esign:    documents → otp_ivr
disburse: account → (done/complete)
```
...
| `api/chat.py` | Core gateway: Proxies to agent on port 8001, resolves widgets based on session state transitions, streams response as SSE |
...
**Purpose:** A modern React chat interface with split-panel layout (chat + journey tracker), voice support, RTL/Arabic handling, and widget rendering.
...
| `src/app/(journey)/layout.tsx` | Split layout with `StepTracker` (5 steps) + `StepCard` on right panel |
| `src/app/(journey)/[product]/page.tsx` | Main chat page. Auth check → load history → `useChat` streaming → voice → widgets |
| `src/app/api/chat/route.ts` | Next.js API route that proxies chat to backend (port 8000) |
...
| `src/components/widgets/*.tsx` | 13 widget components (Nafath, OTP, Offer Slider, etc.) |
...
**Purpose:** A long-running durable workflow that orchestrates the entire loan origination lifecycle via Temporal. Receives signals from the agent and executes activities at each step.
...
| `workflows/rlos_workflow.py` | `RLOSWorkflow` — 5-step workflow with `wait_condition` at each step for signals. Supports escalation at any point. |
...
1. **AWAITING_IDENTITY** → receives `identity_received` signal → runs `mock_nafath_push` activity
2. **AWAITING_OFFER** → receives `offer_selected` signal → runs `mock_simah_pull` activity (credit bureau)
3. **AWAITING_TRADE** → receives `trade_confirmed` signal → waits for customer confirmation
4. **AWAITING_ESIGN** → receives `esign_completed` signal → runs `mock_docusign_send` activity
5. **AWAITING_DISBURSE** → receives `disburse_confirmed` signal → runs `mock_core_banking_transfer` → COMPLETED
